...
router.put('/', async (req, res) => {
    ...
    await userSchema.findOneAndUpdate({ _id: userId }, { $set: { todos: user.todos } }, { new: true });
    let userObj = user.toObject();
    delete userObj.password;
    res.status(200).json({ status: 200, message: 'Todo updated successfully', user: userObj });
});
